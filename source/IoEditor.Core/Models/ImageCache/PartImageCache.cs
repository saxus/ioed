...
namespace IoEditor.Models.ImageCache
{
    internal class PartImageCache
    {
        ...
        {
            _basePath = Path.GetDirectoryName(GetType().Assembly.Location) ?? AppContext.BaseDirectory;
        }
        ...
        private async Task<byte[]?> ReadOrDownloadAsync(string partName, int blColorId)
        {
            byte[]? bytes = null;
