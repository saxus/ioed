...
{
    internal class BADInstructionMerger
    {
        public static void Merge(IoEdProject project)
        {
            ...
            var reindexedStepDictionary = CreateReindexDictionary(stepDictionary);
            InterimStepData GetStepData(int index) => stepDictionary[reindexedStepDictionary[index]];
        ...
        private static Dictionary<int, int> CreateReindexDictionary(Dictionary<int, InterimStepData> stepDictionary)
        {
            return stepDictionary.Keys.Select((k, i) => (index: i, originalIndex: k)).ToDictionary(k => k.index, v => v.originalIndex);
        }
